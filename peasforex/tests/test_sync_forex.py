...
class TestSyncForex(unittest.TestCase):
    ...
    @patch("peasforex.tasks.sync_forex.frappe")
    def test_sync_daily_spot_rates_success(
        self,
        mock_frappe,
        mock_client_class,
        mock_bidirectional,
        ...
        mock_settings.get_enabled_pairs.return_value = [
            {"from_currency": "USD", "to_currency": "UGX", "sync_spot_daily": True}
        ]
        mock_frappe.get_single.return_value = mock_settings
    ...
    @patch("peasforex.tasks.sync_forex.AlphaVantageClient")
    @patch("peasforex.tasks.sync_forex.frappe")
    def test_sync_daily_spot_rates_api_error(
        self, mock_frappe, mock_client_class, mock_sync_log
    ):
