...
def get_data(
    ...
    Supports filtering by currency group (African or Major) and rate type.
    """
    ...
    # Get rates for filtered currency pairs, grouped by date and pair
    query = """
        SELECT 
            rate_date,
    ...
    for pair, date_rates in pairs.items():
        ...
        datasets.append({"name": pair, "values": values})
...
def build_currency_filter(currency_group):
    ...
    if currency_group == "African":
        ...
        return (
            f"AND from_currency IN ('{currencies}') AND to_currency IN ('{currencies}')"
        )
    else:
