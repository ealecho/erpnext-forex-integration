...
class ForexSettings(Document):
    ...
    def test_connection(self):
        ...
        try:
            ...
            if not api_key:
                ...
                return {"status": "error", "message": _("API key is not configured")}
    ...
    def sync_now(self):
        ...
        return {
            "status": "queued",
            "message": _(
                "Daily spot rate sync has been queued. Check Forex Sync Log for status."
            ),
        }
    ...
    def sync_monthly_now(self):
        ...
        return {
            "status": "queued",
            "message": _(
                "Monthly rate sync has been queued. Check Forex Sync Log for status."
            ),
        }

    @frappe.whitelist()
    def backfill_historical(self, months=6):
        ...
        return {
            "status": "queued",
            "message": _(
                "Historical data backfill for {0} months has been queued."
            ).format(months),
        }
