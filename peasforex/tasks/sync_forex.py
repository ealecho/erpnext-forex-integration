...
def log_sync(
    ...
    try:
        from peasforex.peasforex.doctype.forex_sync_log.forex_sync_log import (
            log_sync as _log_sync,
        )

        return _log_sync(
            sync_type, currency_pair, status, exchange_rate, error_message, api_response
        )
    except Exception as e:
...
def update_currency_exchange(
    from_currency, to_currency, rate, date, for_buying=1, for_selling=1, company=None
):
    """
    ...
    """
    log_info(
        f"Updating Currency Exchange: {from_currency} -> {to_currency} @ {rate} on {date} (company: {company or 'default'})"
    )
    ...
    try:
        ...
        if has_company_field:
            ...
            log_debug(
                f"Currency Exchange has company field: {has_company_field}, mandatory: {company_is_mandatory}"
            )
        ...
        if existing:
            ...
            frappe.db.set_value(
                "Currency Exchange", existing, "for_selling", for_selling
            )
...
def create_bidirectional_rate(from_currency, to_currency, rate, date, company=None):
    ...
    log_info(
        f"Creating bidirectional rate: {from_currency} <-> {to_currency} @ {rate} (company: {company or 'default'})"
    )
    ...
    if settings.create_bidirectional_rates and rate > 0:
        reverse_rate = 1 / rate
        log_debug(
            f"Creating reverse rate: {to_currency} -> {from_currency} @ {reverse_rate}"
        )
        update_currency_exchange(
            to_currency, from_currency, reverse_rate, date, company=company
        )
...
def store_rate_log(
    ...
    log_debug(
        f"Storing rate log: {from_currency}-{to_currency} | {rate_type} | {exchange_rate}"
    )

    try:
        from peasforex.peasforex.doctype.forex_rate_log.forex_rate_log import (
            ForexRateLog,
        )
...
def sync_daily_spot_rates():
    ...
    for idx, pair in enumerate(enabled_pairs):
        ...
        log_info(
            f"[{idx + 1}/{len(enabled_pairs)}] Processing {pair_str}"
            + (f" for {target_company}" if target_company else "")
        )
        ...
        try:
            ...
            if settings.auto_update_currency_exchange:
                ...
                try:
                    ...
                    log_error(
                        f"Failed to create Currency Exchange for {pair_str}: {str(e)}"
                    )
    ...
    log_info("=" * 50)
    log_info(
        f"Forex daily sync completed: {success_count} success, {error_count} errors"
    )
    log_info("=" * 50)
...
def sync_monthly_rates():
    ...
    log_info(
        f"Syncing rates for previous month: {first_of_prev_month} to {last_of_prev_month}"
    )
    ...
    for idx, pair in enumerate(enabled_pairs):
        ...
        try:
            ...
            log_error(f"Exception syncing {pair_str}: {str(e)}")
            frappe.log_error(
                frappe.get_traceback(), f"Forex Monthly Sync Error: {pair_str}"
            )
            log_sync(
                sync_type="Closing (Monthly)",
                currency_pair=pair_str,
    ...
    # Calculate PEAS Internal Prudency rates for all pairs
    log_info("Calculating PEAS Internal Prudency rates...")
    calculate_peas_internal_prudency_for_month(
        last_of_prev_month, enabled_pairs, settings
    )

    log_info("=" * 50)
    log_info(
        f"Forex monthly sync completed: {success_count} success, {error_count} errors"
    )
    log_info("=" * 50)


def backfill_historical_rates(months=6):
    """
    Backfill historical exchange rate data for the specified number of months.

    Args:
        months: Number of months to backfill (default: 6)
    """
    log_info("=" * 50)
        ...
        try:
            ...
            for date_str, values in time_series.items():
                try:
                    ...
                    frappe.log_error(
                        f"Error processing {date_str}: {str(e)}", "Forex Backfill Error"
                    )
                    continue
            ...
            frappe.log_error(
                frappe.get_traceback(), f"Forex Backfill Error: {pair_str}"
            )
            log_sync(
                sync_type="Backfill",
                currency_pair=pair_str,
...
def backfill_month_rates(month_date, client, enabled_pairs, settings):
    ...
    for pair in enabled_pairs:
        ...
        try:
            ...
            log_debug(
                f"Processing {len(month_rates)} rates for {pair_str}, closing: {closing_rate}"
            )
            ...
            frappe.log_error(
                frappe.get_traceback(), f"Forex Month Backfill Error: {pair_str}"
            )
            continue

    # After processing all pairs, calculate PEAS Internal Prudency for this month
    calculate_peas_internal_prudency_for_month(month_date, enabled_pairs, settings)


def calculate_peas_internal_prudency_for_month(month_date, enabled_pairs, settings):
    """
    Calculate PEAS Internal Prudency Rate for a specific month.

    Formula: (Average of last 6 Monthly Average rates) × Prudency Factor

    Args:
        month_date: The month to calculate for
        enabled_pairs: List of enabled currency pairs
        settings: Forex Settings object
    """
    from datetime import datetime

    # Get prudency factor from settings (default 0.95)
    prudency_factor = getattr(settings, "peas_prudency_factor", 0.95) or 0.95

    log_info(
        f"Calculating PEAS Internal Prudency for {month_date} with factor {prudency_factor}"
    )
    ...
    for pair in enabled_pairs:
        ...
        try:
            # Get the last 6 Monthly Average rates for this pair
            # Including the current month if available
            ...
            if len(monthly_averages) < 6:
                log_info(
                    f"Skipping PEAS Internal Prudency for {pair_str}: "
                    f"Only {len(monthly_averages)} months of data available (need 6)"
                )
                continue

            # Calculate average of the 6 monthly averages
            rates = [float(r.exchange_rate) for r in monthly_averages]
            six_month_average = sum(rates) / len(rates)

            # Apply prudency factor
            peas_prudency_rate = six_month_average * prudency_factor

            log_debug(
                f"PEAS Prudency for {pair_str}: "
                f"6-month avg={six_month_average:.6f}, "
                f"factor={prudency_factor}, "
                f"result={peas_prudency_rate:.6f}"
            )

            # Store the PEAS Internal Prudency rate
            if settings.store_historical_data:
                store_rate_log(
                    from_currency=from_currency,
                    to_currency=to_currency,
                    rate_date=month_end_str,
                    rate_type="PEAS Internal Prudency",
                    exchange_rate=peas_prudency_rate,
                )

                log_info(
                    f"Stored PEAS Internal Prudency for {pair_str}: {peas_prudency_rate:.6f}"
                )
            ...
            log_error(
                f"Exception calculating PEAS Internal Prudency for {pair_str}: {str(e)}"
            )
            frappe.log_error(
                frappe.get_traceback(), f"PEAS Prudency Calculation Error: {pair_str}"
            )
            continue


def calculate_all_peas_internal_prudency(months=6):
    """
    Calculate PEAS Internal Prudency rates for all months that have sufficient data.

    This function is called manually via the "Calculate PEAS Prudency" button
    or can be run as a standalone function.

    Args:
        months: Number of months to calculate for (default: 6)
    """
    log_info("=" * 50)
    log_info(f"Starting calculate_all_peas_internal_prudency for {months} months")
    log_info("=" * 50)

    if not is_enabled():
        log_error("Forex integration is not enabled")
        frappe.log_error(
            "Forex integration is not enabled", "PEAS Prudency Calculation Skipped"
        )
        return {"status": "error", "message": "Forex integration is not enabled"}
    ...
    if not enabled_pairs:
        log_error("No enabled currency pairs")
        return {"status": "error", "message": "No enabled currency pairs"}
    ...
    for pair in enabled_pairs:
        ...
        try:
            # Get all Monthly Average rates for this pair, ordered by date
            monthly_averages = frappe.db.sql(
                """
                SELECT rate_date, exchange_rate
                FROM `tabForex Rate Log`
            ...
            if len(monthly_averages) < 6:
                log_info(
                    f"Skipping {pair_str}: Only {len(monthly_averages)} months of data (need 6)"
                )
                skip_count += 1
                results.append(
                    {
                        "pair": pair_str,
                        "status": "SKIPPED",
                        "message": f"Only {len(monthly_averages)} months data",
                    }
                )
                continue

            # Calculate PEAS Internal Prudency for each month starting from the 6th
            months_calculated = 0
            for i in range(5, len(monthly_averages)):
                # Get the 6 months ending at this month
                six_months = monthly_averages[i - 5 : i + 1]
                target_date = six_months[-1].rate_date

                # Check if we already have a PEAS Internal Prudency for this month
                ...
                # Calculate average of 6 months
                rates = [float(r.exchange_rate) for r in six_months]
                six_month_average = sum(rates) / len(rates)
                ...
                results.append(
                    {
                        "pair": pair_str,
                        "status": "SUCCESS",
                        "message": f"Calculated {months_calculated} months",
                    }
                )
                log_info(
                    f"Calculated PEAS Internal Prudency for {pair_str}: {months_calculated} months"
                )
            else:
                results.append(
                    {
            ...
            log_error(f"Exception calculating PEAS Prudency for {pair_str}: {str(e)}")
            frappe.log_error(
                frappe.get_traceback(), f"PEAS Prudency Calculation Error: {pair_str}"
            )
            error_count += 1
            results.append(
                {"pair": pair_str, "status": "ERROR", "message": str(e)[:50]}
            )

    # Log summary
    log_info("=" * 50)
    log_info("PEAS INTERNAL PRUDENCY CALCULATION SUMMARY")
    log_info("=" * 50)
    for result in results:
        status_icon = (
            "✓"
            if result["status"] == "SUCCESS"
            else ("⊘" if result["status"] == "SKIPPED" else "✗")
        )
        log_info(
            f"  {status_icon} {result['pair']}: {result['status']} - {result['message']}"
        )
    log_info("=" * 50)
    log_info(
        f"Completed: {success_count} success, {skip_count} skipped, {error_count} errors"
    )
    log_info("=" * 50)

    return {
        "status": "success",
        "message": f"PEAS Internal Prudency calculated: {success_count} success, {skip_count} skipped, {error_count} errors",
        "results": results,
    }
